...
SUBTITLES = [
    ("변기에 묻은 이거 안 생기게 하는 법 알려줄게", 0.4, 3.4),
    ("이건 물때가 아니라 요석이라고 하는 거야", 4.2, 7.0),
    ("그냥 닦으면 금방 다시 생기니까 이번 기회에 배워둬", 7.0, 10.5),
    ("요석은 알칼리성이라 산으로 녹일 수 있어", 11.0, 14.0),
    ("구연산 뿌리고 휴지로 덮어놔", 14.0, 16.2),
    ("기다렸다 벗기면 말끔히 지워져", 18.4, 20.9),
    ("다 됐다. 깨끗하지?", 25.5, 26.9),
]
...
def prepare_cut(index: int, name: str, duration: float, cut_start: float) -> Path:
    ...
    if name == "__timecard__":
        ...
        source = raw
        overlay = drawtext(TIMECARD_TEXT, 0.0, duration, size=56, y=600, box="black@0.6")
    else:
        source = SRC_DIR / name
