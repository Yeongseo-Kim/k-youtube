...
VOICE_DIR = Path("output/ep002/voice_g_fast")   # ElevenLabs v3 Lb7q(G) + 배속 (확정)
...
CUTS = [
    ("cut1.mp4", 2.4, 1.13, 0.0),    # 1a 홀림
    ("cut1d.mp4", 4.3, 2.33, 0.0),   # 상상 — 맥주 망상
    ("cut1.mp4", 1.2, 1.15, 2.6),    # 1b 가격 충격
    ("cut2.mp4", 3.5, 1.14, 0.0),    # 시무룩 (한숨 자리 확보)
    ("cut3.mp4", 4.0, 1.0, 0.0),     # 번뜩 → 아싸
...
SUBTITLES = [
    ("우와… 이 잔 뭐야…", 0.2, 1.9),
    ("보기만 해도 시원하잖아…", 2.1, 3.9),
    ("여기에 맥주 딱 하면…", 4.0, 6.0),
    ("크으~", 6.2, 6.6),
    ("5만 원…?", 7.0, 7.9),
    ("아무리 그래도 잔 하나에 5만 원은…", 9.0, 11.7),
    ("혹시… 인터넷?!", 11.9, 13.6),
    ("쿠팡 2만 원?! 아싸!", 13.8, 15.8),
    ("왔다…!", 17.0, 18.2),
    ("캬아~", 26.4, 27.3),
    ("기다린 보람이 있잖아~!", 27.8, 30.1),
    ("최고!", 30.3, 31.3),
]

EMPHASIS = [
    ("2만 원?!", 13.8, 15.8, 76, "#F28C39", 500),
]
...
SFX_GAIN = 0.85
SFX = [
    ("sfx_sparkle.mp3", 0.4),     # 첫 씬 — 눈 반짝반짝
    ("sfx_harp.mp3", 2.3),        # 상상 진입 하프
    ("sfx_gulp.mp3", 5.5),        # 상상 속 꿀꺽꿀꺽
    ("sfx_scratch.mp3", 6.5),     # 상상 → 현실 추락
    ("sfx_thud.mp3", 6.8),        # 가격 충격 "쿵"
    ("sfx_sigh.mp3", 8.0),        # 한숨 (먹구름 타이밍, 대사 직전)
    ("sfx_ding_idea.mp3", 11.8),  # 전구 띠링
    ("sfx_typing.mp3", 12.6),     # 토도도독 검색
    ("sfx_dingdong.mp3", 15.6),   # 초인종
    ("sfx_rustle.mp3", 16.8),     # 박스 부스럭
    ("sfx_dash.mp3", 19.35),      # 휙
    ("sfx_steps.mp3", 19.45),     # 발소리 (휙 직후)
    ("sfx_can_open.mp3", 20.5),   # 캔 치익
    ("sfx_impact.mp3", 21.1),     # 붓기 임팩트
    ("sfx_pour.mp3", 21.3),       # 따르는 소리
    ("sfx_fizz.mp3", 21.6),       # 탄산 레이어
    ("sfx_gulp.mp3", 25.4),       # 원샷 꿀꺽꿀꺽
    ("sfx_jingle.mp3", 30.1),     # 성공 징글
    ("sfx_sparkle.mp3", 30.3),    # 반짝
    ("sfx_clink.mp3", 30.6),      # 찰랑
]
...
def main() -> Path:
    ...
    mux_audio(silent, mixed, cursor)

    # 최종 전체 배속 — 피치 유지(atempo). 쇼츠 템포용.
    SPEEDUP = 1.12
    run([FFMPEG, "-y", "-v", "error", "-i", str(mixed),
         "-filter_complex",
         f"[0:v]setpts=PTS/{SPEEDUP},fps=30[v];[0:a]atempo={SPEEDUP}[a]",
