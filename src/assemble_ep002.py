...
VOICE_DIR = Path("output/ep002/voice_final")   # v3 감정 태그 + 라인별 배속
...
CUTS = [
    ("cut1.mp4", 2.9, 0.93, 0.0),    # 1a 홀림
    ("cut1d.mp4", 4.3, 2.33, 0.0),   # 상상 — 맥주 망상 (압축)
    ("cut1.mp4", 1.2, 1.15, 2.6),    # 1b 가격 충격 (상상에서 추락)
    ("cut2.mp4", 3.0, 1.33, 0.0),    # 시무룩
    ("cut3.mp4", 3.8, 1.05, 0.0),    # 번뜩 → 아싸
...
SUBTITLES = [
    ("우와… 이 잔 뭐야…", 0.2, 2.0),
    ("보기만 해도 시원하잖아…", 2.2, 4.0),
    ("여기에 맥주 딱 하면…", 4.3, 6.2),
    ("크으~", 6.4, 7.0),
    ("5만 원…?", 7.4, 8.2),
    ("아무리 그래도 잔 하나에 5만 원은…", 8.6, 11.3),
    ("혹시… 인터넷?!", 11.6, 13.0),
    ("쿠팡 2만 원?! 아싸!", 13.2, 15.2),
    ("왔다…!", 16.8, 18.3),
    ("캬아~", 26.5, 27.2),
    ("기다린 보람이 있잖아~!", 27.6, 30.0),
    ("최고!", 30.2, 30.9),
]

# 강조 — 대사에 있는 단어만 키운다 (2만 원 반전): (텍스트, 시작, 끝, 크기, 색, y)
EMPHASIS = [
    ("2만 원?!", 13.2, 15.2, 76, "#F28C39", 500),
]

# 가격표 오버레이 — 상상에서 추락하는 1b 구간에 등장. 컷 확인 후 좌표 조정.
PRICE_TAG = ("₩50,000", 7.2, 8.4, 46, "white", 750)
...
SFX = [
    ("sfx_thud.mp3", 7.3),        # 가격 충격 "쿵" (상상 추락)
    ("sfx_dingdong.mp3", 15.4),   # 초인종
    ("sfx_dash.mp3", 19.2),       # 후다닥
    ("sfx_can_open.mp3", 20.7),   # 캔 칙-
    ("sfx_pour.mp3", 21.4),       # 따르는 소리
    ("sfx_sparkle.mp3", 30.2),    # 최고! 반짝
]
